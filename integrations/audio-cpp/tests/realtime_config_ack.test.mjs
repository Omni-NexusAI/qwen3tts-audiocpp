...

{
  ...
  client._onMicChunk(new Int16Array([1, 2]).buffer);
  assert.deepEqual(sent, [], "microphone PCM must be gated before config acknowledgement");
  ...
  client._onMicChunk(new Int16Array([1, 2]).buffer);
  assert.deepEqual(sent.map((event) => event.type), ["session.update", "input_audio_buffer.append"]);
  await client.close();
}
...
console.log("standalone Realtime pipeline config acknowledgement tests passed");
